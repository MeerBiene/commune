<script>
import { store } from '../../../../store/store.js'
import Popup from '../../../../components/popup/popup.svelte'
import { createEventDispatcher } from 'svelte'
const dispatch = createEventDispatcher()
import { makeid } from '../../../../utils/utils.js'

import { trash, edit, reply } from '../../../../utils/icons.js'

export let event;
export let owner;

let active = false;

function showMenu() {
    if(!active) {
        dispatch('showMenu', true)
    }
    active = true
}

function hideMenu() {
    if(active) {
        dispatch('hideMenu', true)
    }
    active = false
}

let container;


$: account = $store.accounts.filter(account => account.user_id == $store.active_account)[0]

$: matrix = account?.matrix

function redactEvent() {
    console.log("lol")
    let txnID = makeid(32)
    container.kill()

    let content = {
        reason: "deleted",
    }
    matrix.redactEvent(event?.room_id, event?.event_id, txnID, content, (err, res) => {
        console.log(res)
        console.log(res)
        console.log(res)
    });
}

</script>


<Popup 
bind:this={container}
trigger={"click"}
shadow={`0 0px 16px rgba(0,0,0,.5)`}
borderRadius={'4px'}
placement={"left-start"}
on:ready={showMenu}
on:killed={hideMenu}
offset={[-8, 0]}>

    <div class="gr-center et-ic"
        slot="reference"
        aria-label="More"
        data-microtip-position="top"
        data-microtip-size="fit"
        role="tooltip">
        <svg class="icon-3Gkjwa" aria-hidden="false" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M7 12.001C7 10.8964 6.10457 10.001 5 10.001C3.89543 10.001 3 10.8964 3 12.001C3 13.1055 3.89543 14.001 5 14.001C6.10457 14.001 7 13.1055 7 12.001ZM14 12.001C14 10.8964 13.1046 10.001 12 10.001C10.8954 10.001 10 10.8964 10 12.001C10 13.1055 10.8954 14.001 12 14.001C13.1046 14.001 14 13.1055 14 12.001ZM19 10.001C20.1046 10.001 21 10.8964 21 12.001C21 13.1055 20.1046 14.001 19 14.001C17.8954 14.001 17 13.1055 17 12.001C17 10.8964 17.8954 10.001 19 10.001Z"></path></svg>
    </div>

    <div class="tools-container fl-co pa1" slot="content">
        {#if !owner}
            <div class="t-i flex ph2 pv1">
                <div class="flex-one gr-center">
                    Reply
                </div>
                <div class="gr-default gr-center i-c">
                    {@html reply}
                </div>
            </div>
        {/if}
        {#if owner}
            <div class="t-i flex ph2 pv1">
                <div class="flex-one gr-center">
                    Edit Message
                </div>
                <div class="gr-default gr-center i-c">
                    {@html edit}
                </div>
            </div>
        {/if}
        {#if owner}
            <div class="del flex ph2 pv1" on:click={redactEvent}>
            <div class="flex-one gr-center">
                Delete Message
            </div>
            <div class="gr-default gr-center d-i">
                {@html trash}
            </div>
        </div>
        {/if}
    </div>
</Popup>


<style>
.tools-container {
    background-color: var(--dark);
    min-width: 190px;
    color: var(--text);
    line-height: 1.4;
    display: grid;
    overflow: hidden;

}
.et-ic {
    padding: 0.25rem;
    cursor: pointer;
}
.et-ic:hover {
    fill: var(--white);
}
.t-i {
    margin-top: 0.125rem;
    margin-bottom: 0.125rem;
    cursor: pointer;
}

.t-i:hover {
    background-color: var(--green);
}

.del {
    margin-top: 0.125rem;
    margin-bottom: 0.125rem;
    cursor: pointer;
}

.del:hover {
    background-color: var(--secondary);
}

.i-c {
    width: 16px;
    height: 16px;
    fill: var(--text);
}

.d-i {
    width: 16px;
    height: 16px;
    fill: var(--text);
}

</style>
