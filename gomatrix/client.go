...
func (cli *Client) MakeRequest(method string, httpURL string, reqBody interface{}, resBody interface{}) error {
	...
	/*
		...
		ctx, _ = context.WithTimeout(ctx, 21*time.Second)
...
func (cli *Client) JoinRoomWithID(roomID string, content interface{}) (resp *RespJoinRoom, err error) {
	var urlPath string
...
// SetDisplayName sets the user's profile display name. See http://matrix.org/docs/spec/client_server/r0.2.0.html#put-matrix-client-r0-profile-userid-displayname
func (cli *Client) AddRoomAlias(alias, roomID string) (err error) {
...
func (cli *Client) GetProfile(mxid string) (resp *RespProfile, err error) {
...
func (cli *Client) CreatePost(p *CreatePostEvent) (*RespSendEvent, error) {
	...
	if p.IsArticle && len(p.Title) > 0 {
		...
		post.Article.Title = p.Title
		post.Article.Slug = p.Slug

		post.Article.ContentURI = p.ArticleContent

		if len(p.Subtitle) > 0 {
			post.Article.Subtitle = p.Subtitle
		}
		if len(p.Description) > 0 {
			post.Article.Description = p.Description
		}
		if len(p.CanonicalLink) > 0 {
			post.Article.CanonicalLink = p.CanonicalLink
		}
		if p.FeaturedImage != nil &&
			len(p.FeaturedImage.MXC) > 0 {
			post.Article.FeaturedImage = p.FeaturedImage
		}
	}

	if p.Share && p.SharedPost != nil {
		post.SharedPost = p.SharedPost
	}

	if p.ThreadInRoomID != "" {
		post.ThreadInRoomID = &p.ThreadInRoomID
	}

	if p.ThreadRoomAlias != "" {
		post.ThreadRoomAlias = &p.ThreadRoomAlias
	}
	...
	if p.Edit && p.EditEventID != "" {
		post.MRelatesTo = map[string]string{
			"rel_type": "m.replace",
			"event_id": p.EditEventID,
		}
		post.MNewContent = map[string]string{
			"body":    p.Text,
			"msgtype": "m.text",
		}
	}
	...
	if p.ThreadRoomID != "" {
		post.ThreadRoomID = &p.ThreadRoomID
	}
	if p.EventID != "" {
		post.EventID = &p.EventID
	}

	if p.ShareReply && len(p.ReplyPermalink) > 0 {
		t := true
		post.ShareReply = &t
		post.ReplyPermalink = &p.ReplyPermalink
	}
	...
	return cli.SendMessageEvent(p.RoomID, "commune.post", post)
}
...
// ForgetRoom forgets a room entirely. See http://matrix.org/docs/spec/client_server/r0.2.0.html#post-matrix-client-r0-rooms-roomid-forget
func (cli *Client) DeleteRoom(roomID string) (resp *interface{}, err error) {
	u := cli.BuildURL("rooms", roomID)
	err = cli.MakeRequest("DELETE", u, struct{}{}, &resp)
	return
}
...
func (cli *Client) GetNotifications(highlight string) (resp interface{}, err error) {
	query := map[string]string{}
	if highlight != "" {
		query["highlight"] = highlight
	}
	urlPath := cli.BuildURLWithQuery([]string{"notifications"}, query)
	err = cli.MakeRequest("GET", urlPath, nil, &resp)
	return
}

func (cli *Client) GetPushRules() (resp interface{}, err error) {
...
func (cli *Client) RoomState(roomID string) (resp []*Event, err error) {
	u := cli.BuildURL("rooms", roomID, "state")
	err = cli.MakeRequest("GET", u, nil, &resp)
	return
}

func (cli *Client) RoomEvent(roomID, eventID string) (resp *Event, err error) {
	u := cli.BuildURL("rooms", roomID, "event", eventID)
	err = cli.MakeRequest("GET", u, nil, &resp)
	return
}

type RespRelationships struct {
	Events    []Event `json:"events"`
	NextBatch string  `json:"next_batch"`
	Limited   bool    `json:"limited"`
}

func (cli *Client) GetRelationships(req map[string]interface{}) (resp *RespRelationships, err error) {
	u := cli.BuildURL("unstable", "event_relationships")
	err = cli.MakeRequest("POST", u, req, &resp)
	return
}

type ReqSpaces struct {
	MaxRoomsPerSpace int
	AutoJoinOnly     bool
	Limit            int
	Batch            string
}

type RespSpaces struct {
	NextBatch string `json:"next_batch"`
	Rooms     []*PublicRoom
	Events    []*Event
}
...
func (cli *Client) ReportEvent(roomID, eventID string, req *ReqReport) (resp *interface{}, err error) {
	u := cli.BuildURL("rooms", roomID, "report", eventID)
	err = cli.MakeRequest("POST", u, req, &resp)
	return
}
...
func (cli *Client) Messages(roomID, from, to string, dir rune, limit int, filter string) (resp *RespMessages, err error) {
	query := map[string]string{
		"from":   from,
		"dir":    string(dir),
		"filter": filter,
...
func (cli *Client) Search(req interface{}) (resp *RespMessages, err error) {
	urlPath := cli.BuildURL("search")
	err = cli.MakeRequest("POST", urlPath, req, &resp)
	return
}
...
func (cli *Client) Download(fileID, homeserverURL string) ([]byte, error) {
	urlPath := cli.BuildURL("download", homeserverURL, fileID)
	resp, err := cli.Client.Get(urlPath)
	if err != nil {
		return nil, err
	}
...
func (cli *Client) RegisterAvailable(req *ReqRegisterAvailable) (resp *RespRegisterAvailable, err error) {
	urlPath := cli.BuildURLWithQuery([]string{"register", "available"}, map[string]string{
...
func (cli *Client) GetAccountData(userID, data string) (resp interface{}, err error) {
	urlPath := cli.BuildURL("user", userID, "account_data", data)
	err = cli.MakeRequest("GET", urlPath, nil, &resp)
	return
}
...
func (cli *Client) UpdateAccountPreferences(userID string, req interface{}) (err error) {
	urlPath := cli.BuildURL("user", userID, "account_data", "commune.user.preferences")
	err = cli.MakeRequest("PUT", urlPath, req, nil)
	return
}
